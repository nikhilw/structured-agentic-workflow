...
| Verification reports UNDOCUMENTED DRIFT | Something shipped that no decision doc, plan, or Amendment Log records | Close the record before claiming completion: append the decision-doc amendment, add the missing plan entry. The fix for drift is the written record, not a revert. A genuine gap in the *work* goes back to build; a gap in the *record* is closed in writing, now |
